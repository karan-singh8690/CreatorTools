...
export function CombineFiles() {
  ...
  const { history: combineHistory, addHistory, deleteItem, clearHistory, isLoaded: isHistoryLoaded } = useToolHistory('combine', 'Combine PDF')
  ...
  const handleCombine = async () => {
    ...
    if (result) {
      const totalSize = combineFiles.reduce((sum, f) => sum + f.size, 0)
      addHistory(
        `Combined ${combineFiles.length} files into one PDF`,
        { fileCount: combineFiles.length, totalSize, outputFileName: result.name }
      )
      toast({
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 md:px-6 py-3 md:py-4 border-b border-gray-100 bg-white">
        <div>
          <h1 className="text-base md:text-lg font-semibold text-gray-800">Combine Files</h1>
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-y-auto md:overflow-y-hidden">
        ...
        <div className="p-4 md:p-6 md:flex-1 md:min-h-0 md:overflow-auto pb-4 md:pb-0">
          ...
          {combineFiles.length > 0 ? (
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              {/* Desktop header */}
              <div className="hidden md:grid grid-cols-[50px_1fr_100px_100px_140px_50px] gap-2 px-4 py-2 bg-gray-50 border-b border-gray-200 text-xs font-medium text-gray-500">
                <div>No.</div>
                <div>Name</div>
                <div>Page Count</div>
                <div>Page Range</div>
              ...
              {combineFiles.map((file, index) => (
                <div
                  key={file.id}
                  className="grid grid-cols-[50px_1fr_50px] md:grid-cols-[50px_1fr_100px_100px_140px_50px] gap-2 px-4 py-3 border-b border-gray-50 items-center hover:bg-gray-50 transition-colors"
                >
                  ...
                  <div className="hidden md:block text-xs text-gray-500">{file.pages}</div>
                  <div className="hidden md:block text-xs text-gray-500">All</div>
                  <div className="hidden md:block text-xs text-gray-400">{formatDate(file.updatedAt)}</div>
                  <button
        ...
        <div className="w-full md:w-72 bg-white border-t md:border-t-0 md:border-l border-gray-200 p-4 md:p-5 md:overflow-y-auto shrink-0">
          ...
          <div className="space-y-5">
            {/* Ghost Controls — disabled when no files added */}
            <div className={cn("space-y-5", combineFiles.length === 0 && "opacity-50 pointer-events-none select-none")}>
              {/* Output Folder */}
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1.5 block">Output Folder</label>
                <div className="flex gap-2">
                  <Input
                    value="~/Desktop/CreatorTools/Combine"
                    readOnly
            ...
            {combineFiles.length === 0 && (
              <div className="flex flex-col items-center py-4 text-center">
                ...
                <p className="text-xs font-medium text-gray-500">Add files first</p>
                <p className="text-[10px] text-gray-400 mt-1">Settings will activate once files are added</p>
              </div>
            )}
