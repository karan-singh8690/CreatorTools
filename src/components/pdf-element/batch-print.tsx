...
export function BatchPrint() {
  ...
  const { history: printHistory, addHistory, deleteItem, clearHistory, isLoaded: isHistoryLoaded } = useToolHistory('batch-print', 'Batch Print')
  ...
  const handleBatchPrint = async () => {
    ...
    setIsPrinting(false)
    const fileNames = printFiles.map(f => f.name)
    const totalPages = printFiles.reduce((sum, f) => sum + f.pages, 0)
    addHistory(
      `Printed ${printFiles.length} file${printFiles.length !== 1 ? 's' : ''} (${totalPages} pages)`,
      { fileNames, fileCount: printFiles.length, totalPages }
    )
    toast({
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 md:px-6 py-3 md:py-4 border-b border-gray-100 bg-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base md:text-lg font-semibold text-gray-800">Batch PDFs</h1>
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-y-auto md:overflow-y-hidden">
        ...
        <div className="w-full md:w-72 bg-white border-t md:border-t-0 md:border-l border-gray-200 p-4 md:p-5 md:overflow-y-auto shrink-0">
          ...
          <div className="space-y-5">
            {/* Ghost Controls — disabled when no files queued */}
            <div className={cn("space-y-5", printFiles.length === 0 && "opacity-50 pointer-events-none select-none")}>
              {/* Printer */}
              <div>
            ...
            {printFiles.length === 0 && (
              <div className="flex flex-col items-center py-4 text-center">
                <Printer className="w-8 h-8 text-gray-300 mb-2" />
                <p className="text-xs font-medium text-gray-500">Add files first</p>
                <p className="text-[10px] text-gray-400 mt-1">Settings will activate once files are queued</p>
              </div>
            )}
