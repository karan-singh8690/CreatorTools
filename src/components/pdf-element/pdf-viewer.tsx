...
export function PdfViewer() {
  ...
  // Tool history
  const { history: viewerHistory, addHistory, deleteItem, clearHistory, isLoaded: isHistoryLoaded } = useToolHistory('pdf-viewer', 'PDF Viewer')
  ...
  useEffect(() => {
    ...
    const loadPdf = async () => {
      try {
        ...
        if (selectedPdfFile) {
          addHistory(
            `Viewed ${selectedPdfFile.name}`,
            { fileName: selectedPdfFile.name, fileSize: selectedPdfFile.size, pages: doc.numPages }
          )
        }
      } catch (err) {
        if (!cancelled) {
  ...
  }, [selectedPdfFile, pdfJsReady, setPdfPage, setPdfTotalPages, addHistory])
  ...
  return (
    <div className="h-full flex flex-col bg-gray-100">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={cn(
              ...
              showHistory ? 'bg-amber-50 text-amber-500 border border-amber-200' : 'hover:bg-gray-100 text-gray-500'
            )}
            title="View History"
          >
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-y-auto md:overflow-y-hidden">
        ...
        {showPrint && (
          <div className="w-full md:w-64 bg-white border-b md:border-b-0 md:border-r border-gray-200 p-4 md:overflow-y-auto shrink-0">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div
            ref={containerRef}
            className="md:flex-1 md:min-h-0 md:overflow-auto flex items-start justify-center p-4 md:p-6 pb-4 md:pb-0"
          >
            {!selectedPdfFile ? (
              /* No file selected */
          ...
          {/* History Panel (below page navigation) */}
          {showHistory && (
            <div className="border-t border-gray-200 bg-white p-3 shrink-0 max-h-64 overflow-y-auto">
              <ToolHistoryPanel
                history={viewerHistory}
                onDelete={deleteItem}
                onClearAll={clearHistory}
                toolLabel="PDF Viewer"
