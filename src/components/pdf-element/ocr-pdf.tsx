...
export function OcrPdf() {
  ...
  const { history, addHistory, deleteItem, clearHistory, isLoaded } = useToolHistory('ocr', 'OCR PDF')
  ...
  const handleOcr = useCallback(async () => {
    ...
    try {
      ...
      addHistory(
        `OCR processed ${selectedFile.name} — ${result.pages} pages`,
        {
          fileName: selectedFile.name,
          fileSize: selectedFile.size,
          pages: result.pages,
          language: 'English',
          wordCount: result.text.trim().split(/\s+/).filter((w: string) => w.length > 0).length,
        },
        'success'
      )
      toast({
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 md:px-6 py-3 md:py-4 border-b border-gray-100 bg-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base md:text-lg font-semibold text-gray-800">OCR PDF</h1>
        ...
        <div className="w-full md:w-72 bg-white border-t md:border-t-0 md:border-l border-gray-200 p-4 md:p-5 md:overflow-y-auto shrink-0">
          ...
          {!selectedFile ? (
            <div className="space-y-5 opacity-50 pointer-events-none select-none">
              ...
              {/* Output Options (disabled preview) */}
              <div>
